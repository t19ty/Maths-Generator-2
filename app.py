...
<div class="card">
  ...
  <div id="setupBox">
    ...
    <button id="viewResults" onclick="window.location.href='/results'" style="margin-top: 1rem; background: var(--blue);">📊 View My Results</button>
  </div>
...
async function handleAnswer(idx, btn) {
  ...
  const timeTaken = questionStartTime ? (new Date() - questionStartTime) / 1000 : 0;
  ...
  // Submit performance data to database
  try {
    await fetch('/api/submit_answer', {
      ...
      body: JSON.stringify({
        ...
        question: lastQuestion.question,
        userAnswer: lastQuestion.options[idx],
        correctAnswer: lastQuestion.options[lastQuestion.correctIndex],
        isCorrect: correct,
        timeTaken: timeTaken
      })
    });
...
function showScore() {
  ...
  viewResultsBtn.style.cssText = 'margin-top: 1rem; background: var(--blue); width: 100%; padding: 0.6rem 0.8rem; font-size: 1rem; border: 1px solid #ccc; border-radius: var(--radius); color: white; border: none; cursor: pointer;';
  viewResultsBtn.onclick = () => window.location.href = '/results';
  scoreBox.appendChild(viewResultsBtn);
